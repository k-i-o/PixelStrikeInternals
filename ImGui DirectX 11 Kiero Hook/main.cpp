...
HRESULT __stdcall hkPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (tab == 0) {
		if (ImGui::Checkbox("Enable no recoil", &EnableNoRecoil)) {
			weaponUpdate();
		}
		ImGui::SameLine();
		ImGui::SliderFloat("", &recoilValue, 0.0f, 1000.0f);

		if (ImGui::Checkbox("Enable no hip recoil", &EnableNoHipRecoil)) {
			weaponUpdate();
		}
		ImGui::SameLine();
		ImGui::SliderFloat("", &hipRecoilValue, 0.0f, 1000.0f);

		if (ImGui::Checkbox("Enable firerate", &EnableFirerate)) {
			weaponUpdate();
		}
		ImGui::SameLine();
		ImGui::SliderFloat("", &firerate, 0.0f, 1000.0f);
	}
	ImGui::End();
